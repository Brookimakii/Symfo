...
//        ;
//    }
	public function getCartLineByID($idCart, $idProduct): ?CartLine {

		return $this->createQueryBuilder('cartLine')
			->andWhere('cartLine.id = :cartId')
			->andWhere('cartLine.product = :product')
			->setParameter('cartId', $idCart)
			->setParameter('product', $idProduct)
			->getQuery()
			->getOneOrNullResult();
	}
	public function getCartLineByProduct($idCart, $idProduct): ?CartLine {

		return $this->createQueryBuilder('cartLine')
			->andWhere('cartLine.cart = :cartId')
			->andWhere('cartLine.product = :product')
			->setParameter('cartId', $idCart)
			->setParameter('product', $idProduct)
			->getQuery()
